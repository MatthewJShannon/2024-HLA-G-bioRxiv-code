---
title: "SINGLE-CELL RNA-seq TROPHOBLAST AND EXPANDED TROPHOBLAST DATASET PREPARATION"
...
sample1 <- AddMetaData (sample1, metadata = "None",     col.name = "Medications") # Did the patient consume medications during pregnancy?
...
new_interface <- merge (fltsample1, y = c(fltsample2 ,
                                          ...
                                          fltsample28,
                                          fltsample29 ),
                        add.cell.ids  = c("1",  "2",  "3",  "4",  "5",  "6",  "7",              # Beristain   Placenta Data
                                          "8",  "9",  "10", "11", "12", "13", "14", "15", "16", # Vento-Tormo Placenta Data
                                          "17", "18", "19", "22", "23", "25",                   # Vento-Tormo Decidua  Data
                                          "26", "27", "28", "29",                               # Vento-Tormo Decidua  Data
                                          "20", "21", "24"),                                    # Vento-Tormo Blood    Data
                        merge.data    = TRUE,
...
new_interface.list <- new_interface.list [ c("1", "2", "3") ]
...
```{r}
# Integrate the data (continued)
Interface  <- IntegrateData (anchorset            = new_interface.anchors,
                             normalization.method = "SCT",
                             verbose              = FALSE)
...
PCAPlot (Interface, group.by = "Type")
PCAPlot (Interface, group.by = "Phase")
PCAPlot (Interface, group.by = "Source")
```
...
```{r}
Interface <- FindClusters (Interface, resolution = 0.90)
...
TSNEPlot    (Interface, label = TRUE,  pt.size = 2) + NoLegend ()
...
```{r}
# Save the interface  dataset
save (Interface, file = "Interface_Data.Rdata")
```
...
# Label all trophoblast clusters by cell type
       new.cluster.ids       <- c("CTB 1",     # Cytotrophoblast state 1
                                  ...
                                  "TSC",       # hTSC-derived organoid enriched state
...
# Prepare the final trophoblast object

```{r}
# Remove organoid cells
Trophoblasts <- subset (Trophoblasts, subset = Model == "MPI")
Trophoblasts
```
...
## Establish the expanded trophoblast-uterine cell dataset

```{r}
Idents (Interface) <- Interface$ID
BULK_Interface     <- subset (Interface, idents = c("B_P130", "B_P21", "B_P244", "B_P277", "B_P295", "B_P89", "B_P899",
                                                    "VT_D10", "VT_D12", "VT_D9", "VT_P10", "VT_P11", "VT_P12", "VT_P9"))
Idents (BULK_Interface) <- BULK_Interface$celltype
```

```{r}
BULK_Interface$celltype.tissue <- paste       (Idents (BULK_Interface), BULK_Interface$Tissue, sep = "_")
BULK_Interface$celltype        <- Idents      (BULK_Interface)
Idents (BULK_Interface) <- "celltype.tissue"
```

```{r}
Expanded_Trophoblasts <- subset (BULK_Interface, idents = c("CTB_Placenta",  "CTB_Decidua",
                                                            "cCTB_Placenta", "cCTB_Decidua",
                                                            "EVT_Placenta",  "EVT_Decidua",
                                                            "SCTp_Placenta", "SCTp_Decidua",
                                                            "Fibroblast_Decidua",
                                                            "Endothelial cell_Decidua",
                                                            "Epiglandular cell_Decidua",
                                                            "Macrophage_Decidua",
                                                            "uNK_Decidua",
                                                            "cNK_Decidua",
                                                            "T cell_Decidua"))
Expanded_Trophoblasts
```
...
```{r}
Expanded_Trophoblasts <- FindNeighbors (Expanded_Trophoblasts)
Expanded_Trophoblasts <- FindClusters  (Expanded_Trophoblasts, resolution = 0.350)

# Reduce trophoblast data dimensionality
Expanded_Trophoblasts <- RunUMAP (Expanded_Trophoblasts, reduction  = "pca", dims = 1:20)
Expanded_Trophoblasts <- RunTSNE (Expanded_Trophoblasts, reduction  = "pca", dims = 1:20)
Expanded_Trophoblasts
```

# Save the expanded trophoblast object

```{r}
# Save the integrated villous trophoblast and patient organoid and hTSC organoid dataset
save (Expanded_Trophoblasts, file = "Expanded_Trophoblast_Data.Rdata")
```
